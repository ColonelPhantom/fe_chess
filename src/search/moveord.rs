...
pub fn move_priority(
    ...
    b: &mut board::Board,
    tt: &TransTable,
    baseline_eval: crate::eval::ValCp,
    tt_entry: Option<&TtEntry>,
) -> i32 {
    let tt_bonus = match tt_entry {
        Some(e) if e.depthleft > 1 => match e.get_move() {
            None => 0,
            Some(tm) => if tm == *m {
    ...
    let static_eval_score = static_eval_sign * EVAL_WEIGHT
        * (static_eval_eval - baseline_eval as i32);
        
    let check_bonus = CHECK_BONUS
        * match b.is_check(b.side_to_move) {
    ...
    return see::see_capt(b, m, b.side_to_move) as i32 * SEE_WEIGHT
